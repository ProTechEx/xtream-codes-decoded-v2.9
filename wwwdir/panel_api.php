        ...
        switch ($action) {
            ...
            default:
                ...
                foreach ($result['channels'] as $channel) {
                    ...
                    $output['available_channels'][$channel['id']] = array('num' => $channel['live'] == 1 ? $live_num : $movie_num, 'name' => $channel['stream_display_name'], 'stream_type' => $channel['type_key'], 'type_name' => $channel['type_name'], 'stream_id' => $channel['id'], 'stream_icon' => $stream_icon, 'epg_channel_id' => $channel['channel_id'], 'added' => $channel['added'], 'category_name' => $channel['category_name'], 'category_id' => !empty($channel['category_id']) ? $channel['category_id'] : null, 'series_no' => null, 'live' => $channel['live'], 'container_extension' => Dc53AE228DF72D4c140FDa7fd5E7e0BE($channel['target_container']), 'custom_sid' => $channel['custom_sid'], 'tv_archive' => $tv_archive_duration, 'direct_source' => !empty($channel['stream_source']) ? json_decode($channel['stream_source'], true)[0] : '', 'tv_archive_duration' => $tv_archive_duration ? $channel['tv_archive_duration'] : 0);
                }
        }
    } else {
