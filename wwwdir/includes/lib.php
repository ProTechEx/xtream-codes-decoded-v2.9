...
class ipTV_lib
{
    ...
    public static function dC118dd2B94e5F4E1A21651686eD6719()
    {
        ...
        foreach (self::$ipTV_db->get_rows(true, 'id') as $b3c28ce8f38cc88b3954fadda9ca6553 => $B6fc8577128465b7a7ca16798a93f3cd) {
    ...
    public static function F676c856Cdb61b02034dc26129ac9eb5()
    {
        ...
        $Cd4eabf7ecf553f46c17f0bd5a382c46 = self::$ipTV_db->get_row();
        foreach ($Cd4eabf7ecf553f46c17f0bd5a382c46 as $key => $C5805ed257c09a3079ad7fa87c6d5bb2) {
            $output[$key] = $C5805ed257c09a3079ad7fa87c6d5bb2;
    ...
    public static function cAdEb9125b2E81b183688842c5Ac3Ad7($stream_id)
    {
    ...
    public static function a3AE6303492B1F90A30Bd57AD7F17760()
    {
        ...
        foreach (self::$ipTV_db->get_rows() as $row) {
            if ((!empty($row['vpn_ip']) && inet_pton($row['vpn_ip']) !== false)) {
                $e3539ad64f4d9fc6c2e465986c622369 = $row['vpn_ip'];
            }
            else if (empty($row['domain_name'])) { 
                $e3539ad64f4d9fc6c2e465986c622369 = str_replace(array('http://', '/', 'https://'), '', $row['domain_name']);
            } else {
                $e3539ad64f4d9fc6c2e465986c622369 = $row['server_ip'];
            }
            $F53be324c8d9391cc021f5be5dacdfc1 = is_array(self::$settings['use_https']) && in_array($row['id'], self::$settings['use_https']) ? 'https' : 'http';
            $A89ab518408ebebb306a354608eb18cd = $F53be324c8d9391cc021f5be5dacdfc1 == 'http' ? $row['http_broadcast_port'] : $row['https_broadcast_port'];
            $row['server_protocol'] = $F53be324c8d9391cc021f5be5dacdfc1;
            $row['request_port'] = $A89ab518408ebebb306a354608eb18cd;
            $row['api_url'] = $F53be324c8d9391cc021f5be5dacdfc1 . '://' . $e3539ad64f4d9fc6c2e465986c622369 . ':' . $A89ab518408ebebb306a354608eb18cd . '/system_api.php?password=' . ipTV_lib::$settings['live_streaming_pass'];
            $row['site_url'] = $F53be324c8d9391cc021f5be5dacdfc1 . '://' . $e3539ad64f4d9fc6c2e465986c622369 . ':' . $A89ab518408ebebb306a354608eb18cd . '/';
            $row['rtmp_server'] = 'rtmp://' . $e3539ad64f4d9fc6c2e465986c622369 . ':' . $row['rtmp_port'] . '/live/';
            $row['rtmp_mport_url'] = 'http://127.0.0.1:31210/';
            $row['api_url_ip'] = $F53be324c8d9391cc021f5be5dacdfc1 . '://' . $row['server_ip'] . ':' . $A89ab518408ebebb306a354608eb18cd . '/system_api.php?password=' . ipTV_lib::$settings['live_streaming_pass'];
            $row['site_url_ip'] = $F53be324c8d9391cc021f5be5dacdfc1 . '://' . $row['server_ip'] . ':' . $A89ab518408ebebb306a354608eb18cd . '/';
            ...
            $row['server_online'] = in_array($row['status'], $c40aa1cdef4832a8ab2a00328edd21c0) && time() - $row['last_check_ago'] <= 90 || SERVER_ID == $row['id'] ? true : false;
            unset($row['ssh_password'], $row['watchdog_data'], $row['last_check_ago']);
            $f9b9c9baaec5b82b03b15c6eb07ec8f9[intval($row['id'])] = $row;
        }
        return $f9b9c9baaec5b82b03b15c6eb07ec8f9;
    }
    public static function mc_decrypt($d826bb1b5f455613052c5b3b5949121c, $key)
    {
        ...
        $key = pack('H*', $key);
        $D49bad8413a1e326e00365852b39c341 = trim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, $key, $e604917258b158ba003c4d7352099362, MCRYPT_MODE_CBC, $d17b616e2ba1c5acd831fc89992d19b8));
        ...
        $Bf8251826e25f660c6052c269250efc7 = hash_hmac('sha256', $D49bad8413a1e326e00365852b39c341, substr(bin2hex($key), -32));
    ...
    public static function D508D1e2ECC2e304e5BaB85e6a347b23($e3539ad64f4d9fc6c2e465986c622369, $Abc172f3d3864ea14d922df45f999b5c = false)
    {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $e3539ad64f4d9fc6c2e465986c622369);
        ...
        $a4b23a5f1ec2a1b113ea488d60c770d8 = curl_exec($ch);
        $aaafb28417c25d1b17071a7dfdb331d1 = (int) curl_getinfo($ch, CURLINFO_HTTP_CODE);
        curl_close($ch);
    ...
    public static function D0124aFE61d44214b63588b31303A8C4($B13e3f304ca1f14e137f209a5138ea10, $Aa83bd79d8568a4b10bd6fee72c2334c = null, $E2862eaf3f4716fdadef0a008a343507 = 5)
    {
        ...
        foreach ($B13e3f304ca1f14e137f209a5138ea10 as $key => $C5805ed257c09a3079ad7fa87c6d5bb2) {
            if (ipTV_lib::$StreamingServers[$key]['server_online']) {
                $ch[$key] = curl_init();
                curl_setopt($ch[$key], CURLOPT_URL, $C5805ed257c09a3079ad7fa87c6d5bb2['url']);
                ...
                if ($C5805ed257c09a3079ad7fa87c6d5bb2['postdata'] != null) {
                    ...
                    curl_setopt($ch[$key], CURLOPT_POSTFIELDS, http_build_query($C5805ed257c09a3079ad7fa87c6d5bb2['postdata']));
                }
                curl_multi_add_handle($Ca1351297422337811ecdd7968d5753d, $ch[$key]);
            } else {
                $A40568a91a6a25727824068aebbe8d94[] = $key;
            }
        }
        ...
        foreach ($ch as $key => $C5805ed257c09a3079ad7fa87c6d5bb2) {
            $Af301a166badb15e0b00336d72fb9497[$key] = curl_multi_getcontent($C5805ed257c09a3079ad7fa87c6d5bb2);
            if ($Aa83bd79d8568a4b10bd6fee72c2334c != null) {
                $Af301a166badb15e0b00336d72fb9497[$key] = call_user_func($Aa83bd79d8568a4b10bd6fee72c2334c, $Af301a166badb15e0b00336d72fb9497[$key], true);
            }
            curl_multi_remove_handle($Ca1351297422337811ecdd7968d5753d, $C5805ed257c09a3079ad7fa87c6d5bb2);
        }
        foreach ($A40568a91a6a25727824068aebbe8d94 as $key) {
            $Af301a166badb15e0b00336d72fb9497[$key] = false;
    ...
    public static function Cb08394fc6E600fe27F05aEDc447d7bf($key)
    {
        if ($key === '') {
            return '';
        }
...
', $C5805ed257c09a3079ad7fa87c6d5bb2);
    ...
    public static function SaveLog($d887d924b23a04b73a6f893291e44509)
    {
    ...
    public static function E5182E3aFA58Ac7ec5D69d56b28819Cd($b362cb2e1492b66663cf3718328409ad = 10)
    {
        ...
        while ($index < $b362cb2e1492b66663cf3718328409ad) {
